<?php
// $Id: theme-settings.php,v 1.5.2.4 2010/10/30 23:16:50 jmburnz Exp $
...
function pixture_reloaded_form_system_theme_settings_alter(&$form, &$form_state)  {
  ...
  if (theme_get_setting('enable_styles') == 'on') {
    ...
    $form['styles']['font']['headings_styles'] = array(
      ...
      '#default_value' => theme_get_setting('headings_styles'),
      '#options' => array(
        'hs-caps' => t('All caps'),
        'hs-fwn' => t('Font weight normal (default is bold)'),
        'hs-ts' => t('Text Shadow (Block titles only, CSS3)'),
      ),
    );
